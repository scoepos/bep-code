...
lk = d.local_stiffness_matrix(E, I, L_elm)
gk = d.global_stiffness_matrix(damage_off_on, num_el, place_of_damage, lk, dam_var, num_nod)
gk = np.delete(gk, [0, -2], 1)
gk = np.delete(gk, [0, -2], 0)

# making mass matrix
lm = d.local_mass_matrix(A, m, L_elm)
gm = d.global_mass_matrix(lm, num_nod, num_el)
gm = np.delete(gm, [0, -2], 1)
gm = np.delete(gm, [0, -2], 0)

#making damping matrix
s1,s2,w1,w2 = d.getting_damping_coeffients(gk,gm,damping_ratio)
gc = s1*gm + s2*gk
...
print(d.newmark(Mt,Ct,Kt,-F,gamma,beta,dt,z,z_dot,z_dot_dot))
